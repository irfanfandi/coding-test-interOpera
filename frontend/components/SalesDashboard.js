...
export default function SalesDashboard({ selectedUser }) {
  ...
  const sortDeals = useCallback((a, b) => {
    const statusOrder = {
      "Closed Won": 1,
      "In Progress": 2,
      "Closed Lost": 3,
  ...
  const dealsList = useMemo(
    () =>
      selectedUser?.deals?.sort(sortDeals).map((deal, index) => (
        <div
          key={index}
  ...
  return (
    <section className="grid gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
      ...
      <div className="bg-white rounded-2xl shadow p-6">
        ...
        <div className="space-y-3">{dealsList}</div>
      </div>

      {/* Clients */}
      <div className="bg-white rounded-2xl shadow p-6">
        <h2 className="text-xl font-semibold mb-4">Clients</h2>
        <div className="space-y-3">{clientsList}</div>
      </div>
    </section>
  );
}
